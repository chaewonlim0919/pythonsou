...
def myFunc():
    try:
        ...
        # cursor.execute(isql)
        # conn.commit() # 안하면 로컬에만 저장. 해야 원격에 올라감

        '''
        isql = "insert into sangdata values(%s, %s, %s, %s)"
        sql_data = (6, '신상2', 11, 5000)
        # sql_data = 6, '신상2', 11, 5000  # 위와 동일
        ...
        # 자료 수정
        '''
        usql = "update sangdata set sang=%s, su=%s, dan=%s where code=%s"  # pk는 수정안돼
        ...
        print('수정 건수: ', cou)  # 업데이트 성공하면 1이고 실패하면 0. 
        conn.commit()
        '''
