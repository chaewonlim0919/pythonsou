...
from datetime import timedelta  # timedelta: 날짜/시간을 가감해서 기간 설정하기에 효과적

app = Flask(__name__)

# Flask는 세션 사용을 위해  secret_key 설정이 필요
app.secret_key = "abcdef123456"  # 위조 방지용 비밀키값
# 참고 키 값 자동생성 터미널창에서> python -c "import secrets; print(secrets.token_hex(32))"
# 04dd3445ec5835a07191ce5254f71e9300e5326844fb6487c5e7818234878d3e
...
@app.route("/setos")     # route나 get이나 상관없음
def setos():
    favorite_os = request.args.get("favorite_os")

    if favorite_os:
        session.permanent = True    # 세션 만료 시간 적용
        session["f_os"] = favorite_os         # f_os 키로 특정값 session에 저장
