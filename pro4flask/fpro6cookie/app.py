...
# 문자열로 작성한 jinja 템플릿을 렌더링 해 HTML로 반환
# 라우트 함수 이름으로 URL을 안전하게 생성하는 함수

app = Flask(__name__)

# Cookie는 브라우저에 저장되는 작은 키-값 데이터이고, 서버가 클라이언트와 연결 상태를 유지하는 것처럼 할 수 있다.
...
@app.post("/set_cookie")      # 하찮은 정보들 cookie에, 보안이 필요한 정보들 session에
def set_cookie():
    # 쿠키 저장
    name = request.form.get("name", "anonymous")

    # 클라이언트에 쿠키를 심으려면 응답 객체가 필요
    # 먼저 "read_cooki" 페이지로 이동하라"는 redirect 객체를 만들고
